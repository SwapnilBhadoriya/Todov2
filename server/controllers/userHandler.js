...
const getAllTodos = async (req, res) => {
  try {
    const result = await pool.query(
      "Select * from Todos where user_id=$1 order by task_id",
      [req.user.id]
    );
...
const getSingleTodo = async (req, res) => {
  try {
    const result = await pool.query(
      "Select * from Todos where user_id=$1 and task_id=$2",
...
const updateTodo = async (req, res) => {
  try {
    ...
    await pool.query(
      "Update Todos set task=$1 where task_id=$2 and user_id=$3",
      updates
    );
